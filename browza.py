    ...
    if sys.platform == 'darwin':
        ...
        for browser in browsers:
            ...
            if re.match("Firefox", browser):
                browser_list.append(
                    [local_url, 5555, "*custom %s/Contents/MacOS/firefox-bin" % directory_browser, site_url]
                    )
            elif re.match("Safari", browser):
                browser_list.append(
                    [local_url, 5555, "*custom %s/Contents/MacOS/Safari" % directory_browser, site_url]
                    )
            elif re.match("Chrome", browser):
                browser_list.append(
                    [local_url, 5555, "*custom %s/Chrome.app/Contents/MacOS/Chrome" % directory_browser, site_url]
                    )
